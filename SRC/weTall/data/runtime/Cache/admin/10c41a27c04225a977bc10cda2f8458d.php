	...
	<table width="100%" class="table_form">
	    ...
	    <tr>
			...
			<td>
			    ...
			    <?php if(!empty($info['img'])): ?><span class="attachment_icon J_attachment_icon" file-type="image" file-rel="<?php echo attach($info['img'], 'item_cate');?>"><img src="__STATIC__/images/filetype/image_s.gif" /></span><?php endif; ?></td>
		</tr>
		<tr>
			<th><?php echo L('item_cate_type');?> :</th>
            <td>
            	<label><input type="radio" name="type" value="0" <?php if($info["type"] == 0): ?>checked<?php endif; ?>> <?php echo L('item_cate_type_cat');?></label>&nbsp;&nbsp;
              <!--  <label><input type="radio" name="type" value="1" <?php if($info["type"] == 1): ?>checked<?php endif; ?>> <?php echo L('item_cate_type_tag');?></label>-->
            </td>
        </tr>
		<tr>
			<th>首页显示 :</th>
            <td>
                <label><input type="radio" name="is_index" value="0" <?php if($info["is_index"] == 0): ?>checked<?php endif; ?>> 不显示</label>&nbsp;&nbsp;
                <label><input type="radio" name="is_index" value="1" <?php if($info["is_index"] == 1): ?>checked<?php endif; ?>> 显示</label>
            </td>
        </tr>
	    <tr>
			<th>审核状态 :</th>
